/**
 * ETHERNUM v3.8 - Interactive Character Sheet System (Enhanced)
...
function normalizeEthernumAudioUrl(url) {
  ...
  if (ETHERNUM_LEGACY_AUDIO_PATHS.includes(value))
    return ETHERNUM_DEFAULT_AUDIO;
  return value;
}
...
class EthernumApp {
  constructor(config = {}) {
    ...
    const storedAudio =
      localStorage.getItem(`ethernum-music-url-${character}`) ||
      (character === "gyro" ? localStorage.getItem("ethernum-music-url") : "");
    this.config = {
      character,
      backgroundMusicUrl: normalizeEthernumAudioUrl(
        config.backgroundMusicUrl ||
          storedAudio ||
          (character === "gyro" ? ETHERNUM_DEFAULT_AUDIO : ""),
      ),
      enableCustomAudio: config.enableCustomAudio !== false,
      allowMultipleOpenCards:
        config.allowMultipleOpenCards ||
        localStorage.getItem("ethernum-allow-multiple") === "true",
      ...config,
    };
  ...
  init() {
    ...
    console.log("🎯 ETHERNUM v3.8 initialized", {
  ...
  setupBackgroundTrack() {
    if (
      !this.config.enableCustomAudio ||
      localStorage.getItem("ethernum-enable-custom") === "false"
    ) {
      return;
    }
    ...
    try {
      ...
      audio.addEventListener(
        ...
        () => {
          if (
            !audio.src.includes("audio%20%5Bmusic%5D.mp3") &&
            !audio.src.includes("audio%20[music].mp3")
          ) {
  ...
  setupEditMode() {
    ...
    document.addEventListener("ethernum:toggle-edit", () =>
      this.toggleEditMode(),
    );
  }
    ...
    editableSelectors.forEach((selector) => {
      document.querySelectorAll(selector).forEach((el) => {
        el.addEventListener("click", (e) => {
          if (
            this.isEditMode &&
            !el.closest("[data-master-only], .ethernum-master-locked")
          ) {
            e.stopPropagation();
  ...
  toggleEditMode() {
    ...
    if (!indicator) {
      ...
      newIndicator.innerHTML = this.isEditMode
        ? "✎ Edição de Jogador Ativa"
        : "Modo Visualização";
      document.body.appendChild(newIndicator);
    } else {
      indicator.classList.toggle("active");
      indicator.innerHTML = this.isEditMode
        ? "✎ Edição de Jogador Ativa"
        : "Modo Visualização";
    }
    ...
    document
      ...
      ?.classList.toggle("is-active", this.isEditMode);

    console.log(
      this.isEditMode ? "✎ Modo edição ativado" : "🔒 Modo visualização",
    );
    this.saveEditState();
  }
      ...
      if (ikonHead) {
        ...
        if (ikonCard && !ikonCard.dataset.localToggle) {
          this.toggleCard(
            ikonCard,
            "ikon-card",
            this.config.allowMultipleOpenCards,
          );
          this.playSound();
        }
      }
  ...
  detectMasterMode() {
    ...
    if (urlParams.get("master") === "true") {
      ...
      const password =
        localStorage.getItem("ethernum-master-password") || "ethernum-master";
  ...
  addScrollAnimations() {
    ...
    document
      .querySelectorAll(".ikon-card, .rank-card, .prog-card")
      .forEach((card) => {
  ...
  setupMultiSiteIntegration() {
    ...
    window.EthernumAPI = {
      ...
      setMasterMode: (value) => {
        if (
          value &&
          window.EthernumShared?.requestMaster &&
          !window.EthernumShared.isMaster()
        ) {
        ...
        this.masterMode =
          value &&
          localStorage.getItem("ethernum-master-authenticated") === "true";
        this.setupLockedContent();
  ...
  switchSection(sectionId) {
    if (
      window.EthernumShared?.sectionDefinitions?.[this.config.character]?.some(
  ...
  getCharacterData() {
    return {
      name:
        ...
        window.EthernumShared?.characters?.[this.config.character]?.label ||
        "Ethernum",
      level:
        window.EthernumShared?.characters?.[this.config.character]?.level ||
        "-",
      masterMode: this.masterMode,
