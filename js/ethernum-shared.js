(function () {
  ...
  function isPrivateContext() {
    ...
    return (
      path.includes("/pages/ferramentas/mestre-panel.html") ||
      params.get("private") === "true"
    );
  }
  ...
  const characters = {
    ...
    pipping: {
      ...
      overview: {
        ...
        origin:
          "Origem ainda em registro. Use este espaco para a familia Black, a heranca vampirica, a profecia e a primeira noite em que Pipping entendeu o palco.",
    ...
    bayle: {
      ...
      overview: {
        ...
        origin:
          "Origem ainda em registro. Use este espaco para a Porta, a perda de memoria, o primeiro fragmento e o motivo de Bayle ainda insistir em ser humano.",
  ...
  function getSectionLocks(characterId = pageCharacter()) {
    try {
      return (
        JSON.parse(localStorage.getItem(lockKey(characterId)) || "{}") || {}
      );
    } catch {
      return {};
    }
  ...
  function isMaster() {
    return isPrivateContext() && localStorage.getItem(MASTER_KEY) === "true";
  }

  function requestMaster() {
    if (isMaster()) return true;
    const value = window.prompt("Senha do painel privado:");
    if (value === passwordValue()) {
      localStorage.setItem(MASTER_KEY, "true");
      localStorage.setItem("masterMode", "true");
      document.dispatchEvent(
        new CustomEvent("ethernum:master-change", {
          detail: { enabled: true },
        }),
      );
      return true;
  ...
  function leaveMaster() {
    ...
    document.dispatchEvent(
      new CustomEvent("ethernum:master-change", { detail: { enabled: false } }),
    );
  }
  ...
  function applyMasterLocks() {
    ...
    document
      .querySelectorAll(
        "[data-master-only], .secret-seal, .secret-block, .arquivista-box",
      )
      .forEach((el) => {
  ...
  function sectionElements(characterId, sectionId) {
      ...
      return [
        ...
        ...Array.from(
          document.querySelectorAll(
            `.nav-btn[onclick*="'${sectionId.replace("s-", "")}'"]`,
          ),
        ),
        ...(sectionId === "s-ficha"
          ? Array.from(document.querySelectorAll("[data-ethernum-sheet-tab]"))
          : []),
      ].filter(Boolean);
    }
    if (characterId === "pipping") {
      ...
      return [
        ...(selectors[sectionId]
          ? Array.from(document.querySelectorAll(selectors[sectionId]))
          : []),
        ...Array.from(
          document.querySelectorAll(
            `.ethernum-tab-btn[data-filter="${sectionId}"]`,
          ),
        ),
      ];
    }
    if (characterId === "bayle") {
      ...
      return [document.getElementById(`tab-${sectionId}`), tabButton].filter(
        Boolean,
      );
    }
  ...
  function chooseFirstVisible(characterId) {
    ...
    const first = defs.find(
      (section) => !getSectionLocks(characterId)[section.id],
    );
  ...
  function applySectionLocks(characterId = pageCharacter()) {
    ...
    const active = document.querySelector(
      ".section.on, .tab-panel.active, .ethernum-tab-btn.is-active",
    );
    if (
      active?.classList.contains("ethernum-player-hidden") ||
      active?.closest(".ethernum-player-hidden")
    ) {
  ...
  function addPf2Sheet(characterId) {
    ...
    const inventoryCount = Math.max(
      baseInventory.length,
      Number(localStorage.getItem(inventoryKey) || baseInventory.length),
    );
    const inventoryRows = Array.from(
      { length: inventoryCount },
      (_, index) => baseInventory[index] || "",
    );
    const list = (items = []) =>
      items
        .map(
      ...
      <div class="ethernum-pf2-panel is-active" data-pf2-panel="resumo">
        ...
        <div class="ethernum-pf2-stat-grid" style="margin-top:12px">
          ${Object.entries(attrs)
            .map(
              ([name, value]) =>
                `<div class="ethernum-pf2-stat"><span>${name}</span>${editable(`attr-${name}`, value)}</div>`,
            )
            .join("")}
        </div>
      </div>
    ...
    sheet.addEventListener("click", (event) => {
      ...
      if (!tab) return;
      sheet
        .querySelectorAll(".ethernum-pf2-tab")
        .forEach((btn) => btn.classList.remove("is-active"));
      sheet
        .querySelectorAll(".ethernum-pf2-panel")
        .forEach((panel) => panel.classList.remove("is-active"));
      tab.classList.add("is-active");
      sheet
        .querySelector(`[data-pf2-panel="${tab.dataset.pf2Tab}"]`)
    ...
    bindEditable();
    sheet
      .querySelector("[data-add-inventory]")
  ...
  function addCharacterArt(characterId) {
    ...
    if (
      !data?.art ||
      document.querySelector(".ethernum-character-portrait") ||
      document.querySelector(".ethernum-character-overview")
    )
      return;
  ...
  function overviewMarkup(characterId) {
    ...
    return `
      ...
      <div class="ethernum-overview-story">
        <article>
          <p>Origem</p>
          <div class="ethernum-editable" data-private-edit="true" data-edit-key="overview-origin">${overview.origin}</div>
        </article>
        <article>
          <p>Caminho</p>
          <div class="ethernum-editable" data-private-edit="true" data-edit-key="overview-path">${overview.path}</div>
        </article>
      </div>`;
  }

  function addCharacterOverview(characterId) {
    if (!characterId || document.querySelector(".ethernum-character-overview"))
      return;
  ...
  function setupSharedEditMode(characterId) {
    ...
    let editMode = false;
    const selectors =
      "h1, h2, h3, h4, p, li, td, th, .pill, .tag, .badge, .ethernum-editable";
    const save = (el, index) => {
    ...
    const elements = () =>
      Array.from(document.querySelectorAll(selectors)).filter(
        (el) =>
          !el.closest("script, style, .ethernum-shell-nav, .ethernum-tabs"),
      );
    ...
    function toggleEditMode() {
      ...
      document
        .querySelector(".ethernum-edit-btn")
        ?.classList.toggle("is-active", editMode);
    }
  }

  function makePippingTabs() {
    if (
      !location.pathname.toLowerCase().includes("pipping") ||
      document.querySelector(".ethernum-tabs")
    )
      return;
    ...
    tabs.addEventListener("click", (event) => {
      ...
      if (!btn) return;
      tabs
        .querySelectorAll(".ethernum-tab-btn")
        .forEach((b) => b.classList.remove("is-active"));
      btn.classList.add("is-active");
      const filter = btn.dataset.filter;
      document
        .querySelectorAll(
          ".pulso-box, .tier-section, .progression, .footer-quote, .ethernum-pf2-sheet",
        )
        .forEach((el) => {
          const isSheet = el.classList.contains("ethernum-pf2-sheet");
          const isTier =
            el.classList.contains("tier-section") ||
            el.classList.contains("pulso-box");
          const isProgress =
            el.classList.contains("progression") ||
            el.classList.contains("footer-quote");
          el.style.display =
            filter === "all" ||
            (filter === "sheet" && isSheet) ||
            (filter === "tiers" && isTier) ||
            (filter === "progression" && isProgress)
              ? ""
              : "none";
        });
  ...
  function addBayleSheetTab() {
    if (
      !location.pathname.toLowerCase().includes("bayle") ||
      document.getElementById("tab-ficha")
    )
      return;
  ...
  function exposeApi(characterId) {
    ...
    window.EthernumAPI = window.EthernumAPI || {
      getCurrentSection: () =>
        document.querySelector(".tab-panel.active, .section.on, section.active")
          ?.id || "initial",
      switchSection: (sectionId) => routeSection(sectionId),
      getEditMode: () => false,
      setMasterMode: (value) =>
        value ? requestMaster() : (leaveMaster(), false),
      toggleSound: () => null,
      playSound: () => null,
      getCharacterData: () => ({
        ...
        currentSection:
          document.querySelector(
            ".tab-panel.active, .section.on, section.active",
          )?.id || "initial",
      }),
    };
  }
  ...
  function routeSection(sectionId) {
    ...
    const pippingButton = document.querySelector(
      `.ethernum-tab-btn[data-filter="${pippingMap[sectionId] || "all"}"]`,
    );
    if (pippingButton) {
      pippingButton.click();
      return;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("ethernum:master-change", () =>
      applySectionLocks(characterId),
    );
    window.addEventListener("message", (event) => {
      ...
      if (event.data?.action === "SWITCH_SECTION")
        routeSection(event.data.sectionId);
      if (event.data?.action === "APPLY_LOCKS") applySectionLocks(characterId);
      if (
        event.data?.action === "TOGGLE_SOUND" &&
        window.EthernumBackgroundAudio
      ) {
        if (event.data.enabled)
          window.EthernumBackgroundAudio.play().catch(() => null);
